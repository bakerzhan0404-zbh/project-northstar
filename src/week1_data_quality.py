...
def main() -> None:
    ...
    rows = [
        ...
        metric("payment_rows", len(payments), "rows", "payments.csv", "Supplied extract; source-population completeness and sampling are unproven"),
        metric("unique_payments", payments["payment_id"].nunique(), "payments", "payments.csv", "Supplied extract; not a certified enterprise population"),
        ...
        metric("gross_supplied_payment_value_control_total", round(payment_enriched["amount_usd"].sum(), 2), "USD", "calculation", "Within supplied extract and includes every status; not settled value or enterprise total"),
        metric("payment_fee_control_total", round(payments["fee_usd"].sum(), 2), "USD", "payments.csv", "Within supplied extract; estimated fees, not actual bank pricing or enterprise total"),
        metric("manual_touch_payments", payments["manual_touch_flag"].sum(), "payments", "payments.csv", "Within supplied 7,600-record extract; representativeness unproven"),
        metric("manual_touch_rate", round(100 * payments["manual_touch_flag"].mean(), 2), "%", "calculation", "Within supplied 7,600-record extract; not an ACG-wide rate"),
        metric("exception_payments", payments["exception_flag"].sum(), "payments", "payments.csv", "Within supplied 7,600-record extract; representativeness unproven"),
        metric("exception_rate", round(100 * payments["exception_flag"].mean(), 2), "%", "calculation", "Within supplied 7,600-record extract; not an ACG-wide rate"),
        metric("late_release_payments", payments["late_release_flag"].sum(), "payments", "payments.csv", "Within supplied 7,600-record extract; representativeness unproven"),
        metric("late_release_rate", round(100 * payments["late_release_flag"].mean(), 2), "%", "calculation", "Within supplied 7,600-record extract; not an ACG-wide rate"),
        metric("rejected_payments", payments["status"].eq("Rejected").sum(), "payments", "payments.csv", "Within supplied 7,600-record extract; not an ACG-wide rate"),
        metric("pending_payments", payments["status"].eq("Pending").sum(), "payments", "payments.csv", "Within supplied extract; status as-of date and enterprise representativeness are not supplied"),
        ...
        metric("repair_minutes", payments["repair_minutes"].sum(), "minutes", "payments.csv", "Management-estimated effort within supplied extract; representativeness unproven"),
        ...
        metric("manual_local_wire_ach_repair_share", round(100 * payments.loc[manual_payments & manual_core_payment_types, "repair_minutes"].sum() / payments["repair_minutes"].sum(), 2), "% of repair minutes", "calculation", "Manual local transfer, wire, and ACH cohorts within supplied 7,600-record extract; association not causal"),
        ...
        metric("cross_border_payment_fees", round(payments.loc[cross_border_payments, "fee_usd"].sum(), 2), "USD", "calculation", "Within supplied 7,600-record extract; six-month estimated fees, not validated bank pricing"),
        ...
        metric("sap_s4_payment_record_share", round(100 * payment_enriched["erp_system"].eq("SAP-S4").mean(), 2), "% of payments", "calculation", "Within supplied 7,600-record extract; representativeness unproven"),
        ...
        metric("union_atlantic_payment_record_share", round(100 * payment_enriched["bank_name"].eq("Union Atlantic").mean(), 2), "% of payments", "calculation", "Within supplied 7,600-record extract; representativeness unproven"),
