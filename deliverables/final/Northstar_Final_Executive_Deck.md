...
This file is the human-readable source and speaker-note companion to the editable PowerPoint and PDF. The shipped `.pptx` and `.pdf` are built by `src/week4_final_deck.py`, which reuses the Week 2/3 house-style design system in `src/week3_steering_deck.py`. Rebuild with `python3 src/week4_final_deck.py`, then export the PDF with LibreOffice. The structured slide source is `Northstar_Final_Executive_Deck_outline.json` alongside this file.
